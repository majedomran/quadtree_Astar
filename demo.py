...
class MainObject:
    ...
    def _setupgui(self):
        ...
        kernelmenu = OptionMenu(
            frame1, self.kernelsizevar, "13*13", "11*11", "9*9", "7*7", "5*5", "3*3")
        ...
        quadtreebtn = Button(qtframe, text="Generate QuadTree",
                             command=self.onButtonQuadTreePress)
        ...
        label = Label(astarframe, fg='#8080FF',
                      textvariable=self.astarlabelvar)
        label.pack()

        label = Label(rightframe, text="Instructions", font=("Helvetica", 13))
        label.pack()
        label = Label(rightframe, justify=LEFT, text="Generate a random map.\n"
    ...
    def onMouseButton1Press(self, event):
        ...
        path, distances, considered = astar.astar(
    ...
    def onMouseMove(self, event):
        if self.drag_startp:
            self.canvas.coords(self.startpoint, event.x-6,
                               event.y-6, event.x+6, event.y+6)
    ...
    def onButtonQuadTreePress(self):
        ...
        self.qtlabelvar.set("Depth: {}  Nodes: {}".format(
            self.quadtree.depth(), self.quadtree.count()))
        self.astarlabelvar.set("")
        self.pathlabelvar.set("")

        if not self.startpoint:
            pos = MAPSIZE//2
            self.startpoint = self.canvas.create_oval(
                pos-6, pos-6, pos+6, pos+6, fill='#2028FF', width=2)
...
def draw_tile(draw, tile, color):
    draw.rectangle([tile.bb.x, tile.bb.y, tile.bb.x+tile.bb.w,
                    tile.bb.y+tile.bb.h], outline=color)
